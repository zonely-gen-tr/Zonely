...
$script:Translations = @{
    "en_US" = @{
        ...
        "msg.reset_confirm"         = "Are you sure you want to reset the setup? This will delete the domain and database."
        "msg.reset_starting"        = "Resetting setup..."
        "msg.reset_started"         = "Reset started."
        "msg.reset_failed"          = "Reset failed."
    ...
    "tr_TR" = @{
        ...
        "msg.reset_confirm"         = "Kurulumu sıfırlamak istediğinize emin misiniz? Bu işlem domain ve veritabanını siler."
        "msg.reset_starting"        = "Kurulum sıfırlanıyor..."
        "msg.reset_started"         = "Sıfırlama başlatıldı."
        "msg.reset_failed"          = "Sıfırlama başarısız."
        ...
        "menu.reset_setup"          = "Kurulumu Sıfırla"
...
function T([string]$Key, [string]$Fallback, [object[]]$Params) {
    ...
    $allParams = @()
    if ($Params -ne $null) { $allParams += @($Params) }
    if ($args -and $args.Count -gt 0) { $allParams += $args }

    if ($allParams.Count -gt 0) {
        try {
            return $text -f $allParams
        }
        catch {
            return $text
        }
...
function Show-LicenseState([object]$Lic) {
    ...
    if (-not $hasDb) {
        if ($Lic.host -or $Lic.port -or $Lic.username -or $Lic.password -or $Lic.dbname) {
            $hasDb = $true
        }
    }
    if ($hasDb) {
...
function Reset-AutoSetup([string]$Base, [Microsoft.PowerShell.Commands.WebRequestSession]$Session, [int]$LicenseId) {
    $url = Join-Url $Base ($script:ApiEndpoint + "?main=Zonely&container=Renew&licenseId=" + $LicenseId)
    $resp = Invoke-ApiGet -Url $url -Session $Session
    $json = Parse-Json $resp.Content
    return @{ Status = $resp.Status; Raw = $resp.Content; Data = $json }
}
...
function Show-Menu-Auth {
    return Invoke-ArrowMenu (T "menu.main" "Main Menu") ([ordered]@{
            (T "menu.buy" "Buy New License")           = "1"
            (T "menu.list" "List Licenses")            = "2"
            (T "menu.details" "View License Details")  = "3"
            (T "menu.update_domain" "Update Domain")   = "4"
            (T "menu.autosetup" "Start Auto Setup")    = "5"
            (T "menu.dbinfo" "View DB Credentials")    = "6"
            (T "menu.reset_setup" "Reset Setup")       = "7"
            (T "menu.sqldump" "Download SQL Dump")     = "8"
            (T "menu.authme" "Generate AuthMe Config") = "11"
    ...
    while ($true) {
        if (-not $authed) {
            ...
            if ($licenseOps -contains $choice) {
                ...
                elseif ($choice -eq "7") {
                    if (-not (Read-YesNo (T "msg.reset_confirm" "Are you sure you want to reset the setup? This will delete the domain and database."))) {
                        Read-Input (T "prompt.press_enter" "Press Enter to continue...")
                        continue
                    }
                    Write-Info (T "msg.reset_starting" "Resetting setup...")
                    $reset = Reset-AutoSetup -Base $BaseUrl -Session $session -LicenseId $licenseId
                    $ok = $false
                    if ($reset.Data) {
                        if ($reset.Data.error) { $ok = $false } else { $ok = $true }
                        if ($reset.Data.status -and "$($reset.Data.status)".ToLowerInvariant() -eq "started") { $ok = $true }
                    }
                    else {
                        if ($reset.Status -ge 200 -and $reset.Status -lt 300) { $ok = $true }
                    }
                    if ($ok) {
                        Write-Ok (T "msg.reset_started" "Reset started.")
                    }
                    else {
                        Write-Err (T "msg.reset_failed" "Reset failed.")
                    }
                }
                elseif ($choice -eq "8") {
